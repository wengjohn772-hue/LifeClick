...
function MobileShell({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const content = tab === 'checkin' ? <CheckInScreen remaining={remaining} lastCheckIn={lastCheckIn} onCheckIn={checkIn} /> : tab === 'map' ? <MapScreen location={location} message={locationMessage} /> : tab === 'faf' ? <SimpleScreen title="Find a Friend" text="Connect with a friend using a FaF ID and share live safety status." /> : tab === 'feeds' ? <SimpleScreen title="Feeds" text="Community safety updates will appear here." /> : tab === 'safety' ? <SafetyScreen remaining={remaining} lastCheckIn={lastCheckIn} /> : <SettingsScreen user={user} onLogout={onLogout} />;
...
function MapScreen({ location, message }: { location: Location.LocationObject | null; message: string }) {
  return <ScrollView contentContainerStyle={styles.screenContent}><Text style={styles.screenEyebrow}>Live location</Text><Text style={styles.screenTitle}>Map</Text><Text style={styles.screenSubtitle}>{message}</Text><View style={styles.mapPreview}><View style={styles.mapGrid} /><View style={styles.mapPin}><Text style={styles.mapPinText}>⌖</Text></View><Text style={styles.mapPreviewTitle}>{location ? 'Your location received' : 'Satellite preview'}</Text><Text style={styles.mapPreviewText}>{location ? `${location.coords.latitude.toFixed(4)}, ${location.coords.longitude.toFixed(4)}` : 'Native map view will appear here.'}</Text></View></ScrollView>;
}

function SafetyScreen({ remaining, lastCheckIn }: { remaining: number; lastCheckIn: Date | null }) { return <ScrollView contentContainerStyle={styles.screenContent}><Text style={styles.screenEyebrow}>AI safety monitor</Text><Text style={styles.screenTitle}>Safety dashboard</Text><View style={styles.riskCard}><Text style={styles.cardLabel}>CURRENT RISK</Text><Text style={styles.riskLevel}>Low</Text><Text style={styles.cardHint}>No missed check-ins detected.</Text></View><Metric label="Behaviour score" value="100/100" /><Metric label="Missed clicks" value="0" /><Metric label="Next check-in" value={`${Math.ceil(remaining / 60)} min`} /><Metric label="Last signal" value={lastCheckIn ? 'Confirmed' : 'Waiting'} /></ScrollView>; }
